 ...
 * Uses server-side API route for reliable parsing (avoids browser pdfjs-dist issues).
 */
export async function parsePdf(file: File): Promise<ParseResult> {
  try {
    // Use server-side API for PDF parsing to avoid browser bundler issues
...
export async function parsePptx(file: File): Promise<ParseResult> {
  ...
  try {
          ...
          if (content) {
            ...
            if (textMatches) {
              const noteText = textMatches
                .map((match) =>
                  decodeXmlEntities(match.replace(/<\/?a:t>/g, "")),
                )
                .filter((t) => t.trim())
                .join(" ");
              if (noteText.trim()) {
    ...
    if (!text || text.length === 0) {
      warnings.push(
        "No text content extracted from PowerPoint. Slides may contain only images.",
      );
    }
