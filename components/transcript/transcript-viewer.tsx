 ...
 * ```
 */
export const TranscriptViewer = memo(
  function TranscriptViewer({
    ...
    const [internalActiveSegmentIndex, setInternalActiveSegmentIndex] =
      useState<number | undefined>();

    // Use external active segment if provided, otherwise use internal
    const activeSegmentIndex =
      externalActiveSegmentIndex !== undefined
      ...
      <Stack gap="md" className={className}>
        ...
        {!transcript.text && (
          <Alert
            variant="light"
            color="blue"
            title="No transcript text available"
          />
        )}
  ...
  function FullTextView({
    ...
    React.useEffect(() => {
      if (searchQuery && containerRef.current) {
        const currentMatch = containerRef.current.querySelector(
...
const SegmentView = memo(
  function SegmentView({
    ...
    if (segments.length === 0) {
      return (
        <Alert variant="light" color="blue" title="No segments available">
 ...
 */
export const CompactTranscriptViewer = memo(
  function CompactTranscriptViewer({
    transcript,
    className,
 ...
 */
export const SplitTranscriptViewer = memo(
  function SplitTranscriptViewer({
    transcript,
    className,
  }: Omit<TranscriptViewerProps, "defaultView">) {
