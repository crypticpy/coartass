...
import { Box, Text, Badge, Stack, ScrollArea, Center } from '@mantine/core';
...
const SegmentItem = memo(function SegmentItem({
  ...
  return (
    <Box
      ...
      role="listitem"
      tabIndex={0}
      aria-label={`${segment.speaker ? `${segment.speaker}, ` : ''}Jump to ${timestamp}${isActive ? ', currently playing' : ''}`}
    ...
    >
      ...
      {/* Timestamp */}
      <Box style={{ flexShrink: 0, display: 'flex', alignItems: 'flex-start', paddingTop: 2 }}>
        <Text
          component="time"
          size="sm"
          fw={500}
 ...
 */
const VirtualizedSegmentList = memo(function VirtualizedSegmentList({
  segments,
  searchQuery,
  activeSegmentIndex,
  onSegmentClick,
    ...
    if (onSegmentClick) {
      ...
      if (segment) {
        ...
        notifications.show({
          ...
          message: segment.text.slice(0, 60) + (segment.text.length > 60 ? '...' : ''),
          autoClose: 1500,
          color: 'blue',
        });
      }
  ...
  useEffect(() => {
    if (activeSegmentIndex !== undefined && activeSegmentIndex !== null) {
      const segmentArrayIndex = segments.findIndex(s => s.index === activeSegmentIndex);
      if (segmentArrayIndex !== -1) {
        // Check if this is a distant jump (more than 20 segments away from visible area)
        const virtualItems = rowVirtualizer.getVirtualItems();
        const firstVisibleIndex = virtualItems[0]?.index ?? 0;
        const isDistantJump = Math.abs(segmentArrayIndex - firstVisibleIndex) > 20;
          ...
          return (
            ...
            >
              <SegmentItem
                ...
                onClick={
                  onSegmentClick ? () => handleSegmentClick(segment.index) : undefined
                }
              />
            </div>
          );
 ...
 */
const VirtualizedCompactSegmentList = memo(function VirtualizedCompactSegmentList({
  segments,
  searchQuery,
  activeSegmentIndex,
  onSegmentClick,
 ...
 */
export const CompactSegmentList = memo(function CompactSegmentList({
  segments,
  searchQuery,
  activeSegmentIndex,
  onSegmentClick,
  ...
  return (
    <ScrollArea className={className} type="auto" role="list" aria-label="Transcript segments">
      <Stack gap="xs">
        {segments.map((segment) => {
          const isActive = activeSegmentIndex === segment.index;
          const timestamp = formatTimestamp(segment.start);
...
export const GroupedSegmentList = memo(function GroupedSegmentList({
  segments,
  searchQuery,
  activeSegmentIndex,
  onSegmentClick,
  ...
  return (
    <ScrollArea className={className} type="auto" role="list" aria-label="Transcript segments grouped by time">
      <Stack gap="xl">
