...
function VirtualizedTranscriptView({
  ...
  const { startIndex, endIndex, offsetY } = useMemo(() => {
    const start = Math.max(
      0,
      Math.floor(scrollTop / SEGMENT_HEIGHT) - OVERSCAN,
    );
  ...
  useEffect(() => {
    if (
      ...
      const targetScrollTop =
        currentSegmentIndex * SEGMENT_HEIGHT -
        VIEWPORT_HEIGHT / 2 +
        SEGMENT_HEIGHT / 2;
        ...
        <Box style={{ height: totalHeight, position: "relative" }}>
          ...
          >
            {visibleSegments.map((segment, i) => {
              ...
              const annotations = annotationsBySegment?.get(idx) ?? [];
              ...
              return (
                ...
                >
                  ...
                  {hasAnnotations && (
                    <Tooltip label={annotations[0].text} withArrow>
...
interface RadioPlaybackInterfaceProps {
  ...
  /** Map of segment index to annotations */
  annotationsBySegment?: Map<number, TranscriptAnnotation[]>;
}
    ...
    <Stack gap="md" className={className}>
      ...
      <Group gap="md" align="center" justify="space-between" wrap="wrap">
        ...
        <SegmentedControl
          ...
          onChange={(val) =>
            controls.setSpeed(parseFloat(val) as PlaybackSpeed)
          }
      ...
      <style jsx global>{`
        ...
        .transcript-scroll-container {
          ...
          scrollbar-color: var(--mantine-color-gray-5)
        ...
        [data-mantine-color-scheme="dark"] .transcript-scroll-container {
          scrollbar-color: var(--mantine-color-dark-3)
